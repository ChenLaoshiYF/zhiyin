...
纸音就是干这个的——它把系统里老师的声音抓下来，识别成俄语，翻成中文，一行行浮在屏幕角落。你不用打断节奏，眼睛扫过去就懂了。在 RTX 5070 Laptop（16GB 内存）上实测通过，GPU 加速识别 + 本地草稿毫秒级出字。
...
| 环节 | 方案 | 说明 |
|------|------|------|
| 音频捕获 | WASAPI Loopback（soundcard） | 抓系统输出，不依赖麦克风/虚拟声卡 |
| 语音识别 | faster-whisper large-v3-turbo | GPU float16，RTX 50 系实测 |
| 本地翻译 | NLLB-200 distilled 600M | 毫秒级草稿，完全离线 |
| 云端修正 | DeepSeek（OpenAI 兼容） | 术语表 + 上下文润色 |
| 字幕 UI | PyQt6 无边框置顶窗 | 穿透模式 + 可缩放 |
| 打包 | PyInstaller onedir | 双击即玩，无 Python 依赖 |

## 性能基准（RTX 5070 Laptop 实测）

| 指标 | 数值 |
|------|------|
| 首句出字延迟 | < 1.5s（GPU 识别） |
| 本地草稿翻译 | 毫秒级（NLLB-600M） |
| 云端修正延迟 | 1-4s（可关，失败自动回退草稿） |
| 切句粒度 | 静音 1.2s 出句，最长 12s 兜底 |
| 显存占用 | ~2.5GB（识别 + 翻译） |
| 首次启动 | 20-30s（加载模型） |

## 与同类工具的差异

相比同类的实时语音翻译工具（如 Sokuji、My Translator、real_time_speech_translator），纸音专注 **俄→中网课场景**：

- **俄罗斯老师专属**：术语表内置物理课专有名词（欧姆定律、电阻、电压等），针对理工科网课优化
- **真实人声优先**：混合翻译用本地 NLLB 保底 + 云端修正，准确与速度兼得
- **零配置双击即玩**：打包 exe 免 Python 环境，比 Electron 类工具更轻
